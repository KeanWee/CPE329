...
volatile unsigned int SawtoothCount = 0;
...
void GenerateSquareWave()
{
   SquareCount++;                                                       // Increments counter
   if(SquareCount <= dutyMode*5)                                        // Sets voltage to 5V if the counter is 
      TempDAC_Value = 2000;                                             // within the range of the duty cycle

   else
      TempDAC_Value = 0;                                                // Sets voltage to 0V otherwise

   if(SquareCount == 50)                                                // Restricts bounds of counter
      SquareCount = 0;
...
void GenerateSineWave()
{
   Drive_DAC(sineValues[SinCount]+1000);                                // Drives voltage depending on lookup table
   SinCount++;                                                          // Increments counter
   if (SinCount == 50)                                                  // Restricts range of counter
     SinCount = 0;
}
...
void GenerateSawtoothWave()
{
   if(SawtoothCount == 0)                                               // Sets voltage to 5V at start
      TempDAC_Value = 2000;
   ...
   SawtoothCount++;                                                     // Increments counter

   if(SawtoothCount == 50)                                              // Restricts range of counter
      SawtoothCount = 0;
}
